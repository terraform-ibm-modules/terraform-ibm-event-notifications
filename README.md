...
* [Examples](./examples)
    * [IBM Cloud Framework for Financial Services profile example](./examples/fscloud)
    * [Basic example](./examples/basic)
    * [Complete example with bring your own key encryption and context-based restrictions rules](./examples/complete)
...
| <a name="input_root_key_id"></a> [root\_key\_id](#input\_root\_key\_id) | The key ID of a root key, existing in the KMS instance that is passed in `var.existing_kms_instance_crn`, which is used to encrypt the data encryption keys which are then used to encrypt the data. Required if the `var.kms_encryption_enabled` input is set to `true`. | `string` | `null` | no |
...
| <a name="input_skip_iam_authorization_policy"></a> [skip\_iam\_authorization\_policy](#input\_skip\_iam\_authorization\_policy) | Set to `true` to skip the creation of an IAM authorization policy that permits all Event Notifications instances in the resource group to read the encryption key from the KMS instance. No policy is created if `var.kms_encryption_enabled` variable is set to `false`. | `bool` | `false` | no |
| <a name="input_tags"></a> [tags](#input\_tags) | The list of tags to be added to the Event Notifications instance. | `list(string)` | `[]` | no |
