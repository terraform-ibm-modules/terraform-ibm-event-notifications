...
locals {
  ...
  en_key_name = (var.prefix != null && var.prefix != "") ? "${var.prefix}-${var.en_key_name}" : var.en_key_name
  # If a prefix value is passed, add it to the EN key ring name
  en_key_ring_name = (var.prefix != null && var.prefix != "") ? "${var.prefix}-${var.en_key_ring_name}" : var.en_key_ring_name
...
locals {
  ...
  cos_bucket_name             = var.existing_cos_bucket_name != null ? var.existing_cos_bucket_name : local.create_cos_bucket ? ((var.prefix != null && var.prefix != "") ? "${var.prefix}-${var.cos_bucket_name}" : var.cos_bucket_name) : null
...
module "event_notifications" {
  ...
  resource_group_id        = module.resource_group[0].resource_group_id
  region                   = var.region
  name                     = (var.prefix != null && var.prefix != "") ? "${var.prefix}-${var.event_notification_name}" : var.event_notification_name
