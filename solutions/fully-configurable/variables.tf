...
variable "cos_key_ring_name" {
  ...
  description = "The name of the key ring which will be created for Object Storage. Not used if supplying an existing key. Only use if 'enable_collecting_failed_events' is true. If a `prefix` input variable is specified, it is added to this name in the `<prefix>-value` format."
}
